...
export class AutonomousLoop {
  ...
  public async executeCycle(
    ...
    try {
      ...
      console.error(`[AutonomousLoop.executeCycle] Failed for mission ${mission.id} (trace ${traceId}):`, err);

      try {
        messageBus.publish('SYSTEM_ALERT', 'AutonomousLoop', {
          message: `Autonomous loop cycle failed for mission '${mission.title || mission.objective}': ${errMsg}`,
          missionId: mission.id,
          traceId,
        }, { missionId: mission.id, severity: 'error' });
      } catch {
        // Bus publish is best-effort — never mask the original failure
      }
      ...
      return {
        ...
        phase: 'EXECUTE',
        status: 'FAILED',
        message: `Autonomous cycle failed: ${errMsg}`,
        timestamp: new Date().toISOString(),
      };
    }
  }
