...
export class TaskRepository {
  ...
  upsert(task: TaskRecord): void {
    try {
      ...
      console.error(`[TaskRepository.upsert] Failed for task ${task.taskId}:`, err);
      throw err;
    }
  }
  ...
  findQueuedTasks(limit: number, kinds?: string[]): TaskRecord[] {
    try {
      ...
      console.error(`[TaskRepository.findQueuedTasks] Failed (limit=${limit}, kinds=${kinds?.join(',')}):`, err);
      throw err;
    }
  }
  ...
  updateStatus(taskId: string, status: TaskQueueStatus): void {
    try {
      ...
      console.error(`[TaskRepository.updateStatus] Failed for task ${taskId}:`, err);
      throw err;
    }
  }
  ...
  updateResult(
    ...
    try {
      ...
      console.error(`[TaskRepository.updateResult] Failed for task ${taskId}:`, err);
      throw err;
    }
  }
  ...
  recordError(taskId: string, error: string, attempt: number): void {
    try {
      ...
      console.error(`[TaskRepository.recordError] Failed for task ${taskId}:`, err);
      throw err;
    }
  }
  ...
  get(taskId: string): TaskRecord | undefined {
    try {
      ...
      console.error(`[TaskRepository.get] Failed for task ${taskId}:`, err);
      throw err;
    }
  }
  ...
  query(filter: TaskFilter = {}): TaskRecord[] {
    try {
      ...
      console.error('[TaskRepository.query] Failed:', err);
      throw err;
    }
  }
  ...
  getStats(): {
    ...
    try {
      ...
      console.error('[TaskRepository.getStats] Failed:', err);
      throw err;
    }
  }
  ...
  upsertWorker(reg: WorkerRegistration): void {
    try {
      ...
      console.error(`[TaskRepository.upsertWorker] Failed for worker ${reg.workerId}:`, err);
      throw err;
    }
  }
  ...
  getWorkers(): any[] {
    try {
      ...
      console.error('[TaskRepository.getWorkers] Failed:', err);
      throw err;
    }
  }
  ...
  private rowToTask(row: any): TaskRecord {
    ...
    return {
      ...
      prompt: row.prompt,
      requiredCapabilities: parse(row.required_capabilities, []) as string[],
      timeoutMs: row.timeout_ms,
      retryPolicy: parse(row.retry_policy, undefined),
      ...
      errorHistory: parse(row.error_history, []) as { timestamp: string; error: string; attempt: number }[],
      agentRole: row.agent_role,
      agentCapabilities: parse(row.agent_capabilities, undefined) as string[] | undefined,
    } as TaskRecord;
  }
}
